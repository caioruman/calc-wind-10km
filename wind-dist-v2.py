...
'''
  - Extract the data from the model and divide it by the sensible heat flux
  - Interpolate to the same vertical level
'''
...
#          mslp = r.variables['PN'][:]
        ...
        for lat, lon, name in zip(lats, lons, stnames):
          ...
          tt_neg_interp = interpolateData(neg_tt_model, new_levels, neg_gz_tt)
          tt_pos_interp = interpolateData(pos_tt_model, new_levels, pos_gz_tt)

          saveDataframe(folder, name, year, month, m, levels, neg_dates, pos_dates, tt_neg_interp, tt_pos_interp, 'temp', None, [neg_t2m, pos_t2m], [neg_stemp, pos_stemp])

          # Wind

          wind_neg_interp = interpolateData(neg_wind_model, new_levels, neg_gz_uu)
          wind_pos_interp = interpolateData(pos_wind_model, new_levels, pos_gz_uu)

          saveDataframe(folder, name, year, month, m, levels, neg_dates, pos_dates, wind_neg_interp, wind_pos_interp, 'wind', [neg_wind, pos_wind])
          ...
          p_neg_interp = interpolateData(neg_p, new_levels, neg_gz_tt)
          p_pos_interp = interpolateData(pos_p, new_levels, pos_gz_tt)

          saveDataframe(folder, name, year, month, m, levels, neg_dates, pos_dates, p_neg_interp, p_pos_interp, 'pressure')
